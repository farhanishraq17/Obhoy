...
export const MemberEnrollment: React.FC = () => {
  ...
  const handleEnroll = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 5. Create new UserProfile with connected Holder Number & NID
      ...
      // 6. Register into authStore and persistent local storage
      registerUser(newProfile);

      // 7. Add policy into simulationStore so policyholder view renders it immediately
      ...
      showToast(`Member enrolled! Assigned Holder Number: ${holderNumber}`, 'success');
    } catch (err: any) {
      setIsProcessing(false);
      setErrorMessage(err.message || 'Enrollment registration failed.');
    }
  };
          ...
          <div className="space-y-6">
            <div className="text-center space-y-2">
              ...
              <h1 className="text-2xl sm:text-3xl font-extrabold text-slate-900 tracking-tight">
                Enroll New Policyholder Member
              </h1>
              <p className="text-xs sm:text-sm text-slate-600 max-w-md mx-auto">
                Enter your National ID (NID) and information to generate your secure <strong>Holder Number</strong>.
              </p>
            </div>

            {errorMessage && (
              <div className="p-4 rounded-xl bg-rose-50 border border-rose-200 text-rose-800 text-xs font-mono">
                <strong>Enrollment Error:</strong> {errorMessage}
              </div>
            )}
